...
const levelIndicator = new Text([10, 10], "", {
    font: "https://fonts.gstatic.com/s/chango/v7/2V0cKI0OB5U7WaJCyHe5.woff2",
    fill: "#222",
    fontSize: 20,
});
...
function setup () {
    ...
    if (currentData) {
        ...
        (currentData.walls || []).forEach((data) => {
            const positions = data.map(pos => new Position(...pos));
            const wall = new Wall(...positions, Wall.types.unmovable);
            walls.add(wall);
            scene.add(wall);
        });
    }
    ...
    .on(Scene.events.draw, () => {
        if (controller.state === Controller.PLAY) {
            ...
            if (ball.previousPosition) {
                ball.position.add(ball.position.clone().subtract(ball.previousPosition));
                ball.options.rotation += (ball.position.x - previous.x) / 100;
            }
            ...
            ball.previousPosition = previous;
